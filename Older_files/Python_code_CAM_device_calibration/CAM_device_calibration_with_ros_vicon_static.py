...
class read_data():
    ...
    def callback(self, msg):
		...
		self.data.linear.x = 1.15*(encoder_phi) + 3
		self.data.linear.y = encoder_theta - 4
		...
		if flag_header == 0:
			...
			# header = 'encoder_phi, encoder_theta\n'
			file_name = "static_trial_24_july_2020.csv"
			data_file = open(file_name, "a")
			data_file.write(str(current_time) + "," + str(encoder_phi) + ',' + str(encoder_theta) + ',' + str(phi) + ',' + str(theta) + '\n')
			data_file.close()
